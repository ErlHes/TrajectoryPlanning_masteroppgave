...
%     Nvr = -5888.89; % Kgm
    ...
    % System dynamics.
    R = [cos(ssa(x(3)))    -sin(ssa(x(3)))    0;...
         sin(ssa(x(3)))    cos(ssa(x(3)))     0;...
            0           0       1];    
    M = [m11   m12     m13;...
         m21   m22     m23;...
         m31   m32     m33];
...
%     Kt = 10^2;
    R2 = [cos(ssa(x(3)))    -sin(ssa(x(3)));...
         sin(ssa(x(3)))    cos(ssa(x(3)))];
    ...
    L = Error'* Kp * Error + Ku * (x(4)-xref(4))^2 + Kv * (x(5)-xref(5))^2 + Kfy * tau(2)^2;% + Kr * (x(6) - xref(6))^2 + Kt * (tau'*tau) + Ku * (x(4) - xref(4))^2;
