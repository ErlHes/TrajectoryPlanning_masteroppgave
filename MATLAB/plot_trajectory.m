function handle_ = plot_trajectory(agent, option, scale)
    ...
    if isfield(agent,'nu')
        dash_thickness = agent.nu(1)*2 * scale + 0.1;
%         dash_end = dash_orig + agent.nu(1)/2 *dash_direction;
    else
        dash_thickness = agent.vel*2* scale + 0.1;
%         dash_end = dash_orig + agent.vel/2 * dash_direction;
    end
    dash_start = dash_orig - scale * dash_direction;
    dash_end = dash_orig + scale * dash_direction;
