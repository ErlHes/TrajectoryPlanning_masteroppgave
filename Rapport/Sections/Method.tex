...
Emil Thyri, the MATLAB simulator's developer, as it is a necessity for simulating \gls{Ts}s. Therefor this chapter will be brief. The logic implemented
...
has the x and y-axis opposite from what we have gotten used to by now. The implementation for calculating the angle $\gamma_p$ (here called pi\_p) is:
\begin{lstlisting}
...
\end{lstlisting}
where pos is the position of the OS. But instead of all this you could use the bearing from the \gls{OS} to the intersection point
to know which scan line resulted in said intersection, each scan line can only have one orthogonal vector, and they can be pre-calculated and put in a list. 
You then only have to grab the correct index from the list to get your angle pi\_p. The constraint function placed in $\textbf{g}(\bm{\omega})$ 
is Equation \eqref{EQ: Static ye}, with the lower bound value for g deciding how close the \gls{OS} is allowed the line. 
In this thesis that lower bound is hard-coded to be 5 meters, but just as with dynamic obstacles
