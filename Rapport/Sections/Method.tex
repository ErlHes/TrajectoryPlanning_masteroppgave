...
Then the algorithm iterates through all the control intervals, $NP$, from $K = 0:NP-1$, as decided by discretization step length and time horizon, 
...
% (TODO: IKKE FERDIG:)\newline
% In the final version of the trajectory planning algorithm there are three major and five minor tasks in the setup:\newline
% Declare and initialize persistent variables (minor. This includes hardcoding if needed for debugging.)\newline
% Initialize dynamic obstacles if there are Tracks present, and simplify the tracks if the settings demand it. (minor)\newline
% Conduct COLREGs classification (major).\newline
% Calculate dynamic horizon (major). \newline
% Run through the CasADi initialization. (major).\newline
% Conduct feasibility check if it's not the very first time the algorithm is run. (minor, very important).\newline
% Sanitize the initial position to get rid of WrapTo2Pi problems. (minor, very important).\newline
% Fetch static obstacles data(very minor, only due to the setup of the MATLAB simulator).
...
% \end{itemize}
The dynamic horizon algorithm could also theoretically calculate a desired discretization step length, but in the current version of the algorithm
that functionality is hardcoded.
...
\subsection{NLP Construction and Solver} (TODO: ??? Dette kan ikke være bare et kapittel.)
% \begin{itemize}
